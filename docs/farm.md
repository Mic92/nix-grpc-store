...
services.nix-grpc-daemon = {
  ...
  accessRules = [
    ...
    { cn = "lb-*"; role = "trusted"; }   # scheduler nodes: envoy asks for the builder list
  ];
  niks3 = {
...
| `advertise` | The address the balancer reaches this node at. A builder's must be `IP:port`, since the scheduler passes it to envoy as an endpoint. A scheduler node's must match its entry in the balancer's `scheduler`. |
...
| `scheduler` | The node(s) with the `scheduler` role, as in their `advertise`. With several, envoy routes to the active one and asks it for the builders. Envoy needs this list before it has asked anybody. |
