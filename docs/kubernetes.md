...
group's `system`, so `workers.<group>.system` is required. A replacement
proxy reads the list over its own connection to the scheduler and answers
that path from clients with an error.
