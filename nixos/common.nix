# Options and settings shared by the NixOS and nix-darwin modules.
{
  config,
  lib,
  pkgs,
  ...
  options.services.nix-grpc-daemon = {
    ...
    listen = lib.mkOption {
      ...
      description = ''
        ...
        systemd socket unit when {option}`idleTimeout` is set, otherwise
        by the daemon.
      '';
    };
    ...
    idleTimeout = lib.mkOption {
      ...
      description = ''
        ...
        runs the daemon as a plain service that binds {option}`listen` itself.
      '';
    };
