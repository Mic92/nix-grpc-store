...
in
{
  options.services.nix-grpc-daemon = {
    ...
    roles = lib.mkOption {
      ...
      description = ''
        `builder` runs the builds the scheduler assigns to it. `scheduler`
        keeps the queue for all systems. Exactly one node per farm has it.
        A single node keeps both and schedules onto itself.
      '';
    };
    scheduler = lib.mkOption {
      ...
      description = ''
        Where a builder opens its `WorkerSession`: the scheduler node or the
    ...
    schedulerCaFile = lib.mkOption {
      ...
      default = null;
      description = ''
        Talk TLS to {option}`scheduler` and verify it against this CA. The
        node's own {option}`tls.certFile` is the client certificate.
        Plaintext if unset.
      '';
    };
    advertise = lib.mkOption {
      ...
      description = ''
        `IP:port` the balancer knows this worker by. Must match its entry in
        the balancer's `workers`. Defaults to {option}`listen`.
      '';
    };
    maxJobs = lib.mkOption {
      type = lib.types.ints.between 0 63;
      default = 1;
      description = "Concurrent builds on this node.";
    };
    minFree = lib.mkOption {
      type = lib.types.str;
      default = "10G";
      description = ''
        Below this much free space under /nix the builder tells the scheduler
        to stop assigning to it. `0` disables.
      '';
    };
    ...
    niks3 = lib.mkOption {
      default = null;
      description = ''
        Publish everything this node imports or builds to a niks3 binary
        cache and substitute from it. Required once there is more than one
        worker. Optional on a single node.
      '';
      type = lib.types.nullOr (
        lib.types.submodule {
          options = {
            ...
            cacheUrl = lib.mkOption {
              ...
              description = "Binary cache niks3 publishes to. Nodes substitute inputs from it.";
            };
  ...
  config = lib.mkIf cfg.enable {
    ...
    # Builds go in by drv path under our uid; the gRPC access rules decide
    # who may, so the proxy itself has to be allowed to.
    ...
    nix.settings.max-jobs = lib.mkIf builder (lib.mkDefault (lib.max 1 cfg.maxJobs));
    ...
    systemd.services.nix-grpc-daemon = {
      ...
      # "Reload" drains: the worker leaves the balancer, exits once running
      # builds have published and the socket starts the new generation. A
      # deploy thus never waits for or kills builds.
      reloadIfChanged = builder;
      serviceConfig = {
        Type = "notify";
        WatchdogSec = 30;
        # Builds in nix-daemon.service must not starve RPCs and the scheduler.
        ...
        ExecStart = lib.escapeShellArgs (
          ...
          ++ lib.optionals (cfg.schedulerCaFile != null) [
            "--scheduler-ca"
            cfg.schedulerCaFile
          ]
      ...
      // lib.optionalAttrs builder {
        ...
        # `systemctl stop` drains too; niks3 push must survive to publish.
        KillMode = "mixed";
