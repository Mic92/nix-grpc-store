  ...
  options.services.nix-grpc-daemon = {
    ...
    scheduler = lib.mkOption {
      ...
      description = ''
        ...
        balancer. `null` uses the in-process scheduler. TLS with
        {option}`tls.clientCaFile` and the node's own certificate if set.
      '';
    };
    ...
    maxJobs = lib.mkOption {
      ...
      default = null;
      description = "Concurrent builds on this node. Defaults to the local `nix.settings.max-jobs`.";
    };
