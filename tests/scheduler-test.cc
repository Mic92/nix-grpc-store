...
auto hello(Shard & shard, std::string_view addr, uint32_t jobs, std::vector<std::string_view> running = {}, std::vector<std::string> features = {}) -> WorkerId
{
    return shard.hello({.addr = addr, .features = std::move(features), .maxJobs = jobs, .running = std::move(running)});
}
...
void testAssignAndDedup()
{
    ...
    shard.want(1, {.drvPath = "a.drv", .inputs = {}, .features = {}, .cpHintMs = 0}, 0);
    shard.want(2, {.drvPath = "a.drv", .inputs = {}, .features = {}, .cpHintMs = 0}, 0);
    shard.dispatch(out);
    ...
    auto res = shard.want(3, {.drvPath = "a.drv", .inputs = {}, .features = {}, .cpHintMs = 0}, 0);
...
void testMostFreeAndLocality()
{
    ...
    shard.want(1, {.drvPath = "b.drv", .inputs = {"/nix/store/big"}, .features = {}, .cpHintMs = 0}, 0);
    shard.dispatch(out);
    assert(out.size() == 1 && out[0].worker == w1);
    // without locality: most free (w0 has 4, w1 has 3)
    shard.want(1, {.drvPath = "c.drv", .inputs = {}, .features = {}, .cpHintMs = 0}, 0);
    shard.dispatch(out);
...
void testPriority()
{
    Shard shard;
    std::vector<Assign> out;
    shard.want(1, {.drvPath = "low.drv", .inputs = {}, .features = {}, .cpHintMs = 0}, 0);
    shard.want(1, {.drvPath = "high.drv", .inputs = {}, .features = {}, .cpHintMs = 100000}, 0);
    shard.want(2, {.drvPath = "fan.drv", .inputs = {}, .features = {}, .cpHintMs = 0}, 0);
    shard.want(3, {.drvPath = "fan.drv", .inputs = {}, .features = {}, .cpHintMs = 0}, 0);
    shard.want(4, {.drvPath = "fan.drv", .inputs = {}, .features = {}, .cpHintMs = 0}, 0);
    shard.want(5, {.drvPath = "fan.drv", .inputs = {}, .features = {}, .cpHintMs = 0}, 0);
    shard.want(6, {.drvPath = "fan.drv", .inputs = {}, .features = {}, .cpHintMs = 0}, 0); // 4 extra followers * 30e3 = 120e3 > 100e3
    hello(shard, "w", 1);
    shard.dispatch(out);
...
void testHelloReconciles()
{
    ...
    auto res = shard.want(1, {.drvPath = "a.drv", .inputs = {}, .features = {}, .cpHintMs = 0}, 0);
    assert(res.assigned && res.assigned->first == w0);
    shard.want(1, {.drvPath = "b.drv", .inputs = {}, .features = {}, .cpHintMs = 0}, 0);
    shard.want(1, {.drvPath = "c.drv", .inputs = {}, .features = {}, .cpHintMs = 0}, 0);
    shard.dispatch(out);
...
void testFeatures()
{
    ...
    shard.want(1, {.drvPath = "vm.drv", .inputs = {}, .features = {"kvm"}, .cpHintMs = 9e9}, 0);
    shard.want(1, {.drvPath = "vm2.drv", .inputs = {}, .features = {"kvm"}, .cpHintMs = 9e9}, 0);
    shard.want(1, {.drvPath = "a.drv", .inputs = {}, .features = {}, .cpHintMs = 0}, 0);
    shard.want(1, {.drvPath = "gpu.drv", .inputs = {}, .features = {"cuda"}, .cpHintMs = 0}, 0);
