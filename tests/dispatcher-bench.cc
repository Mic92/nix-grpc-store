// Benchmark for the Dispatcher without gRPC.
//
// A generated, nixpkgs-shaped build graph is played against one Dispatcher
// in simulated time. Simulated clients do what GrpcStore::buildPaths does:
// send Want for every derivation whose inputs are built, start the build
// when Assigned arrives, and unlock dependents when it finishes. Simulated
// workers answer each Expect with Done after the derivation's duration.
// What is measured is host CPU time spent inside Dispatcher calls, per
...
//
// Output is logfmt on stdout. Any single Dispatcher call that takes longer
// than 1 ms is reported as event=spike with CLOCK_MONOTONIC bounds, so
// `perf record -k CLOCK_MONOTONIC -g -- dispatcher-bench firehose` and
// `perf report --time a,b` can show where it went.
//
// Note on spikes: with glibc malloc a few remain at firehose scale. They
// are heap consolidation inside operator new after ~10^6 frees, not
// scheduler work, and disappear with LD_PRELOAD=libjemalloc.so.
