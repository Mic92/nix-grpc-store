...
void testPush(Suite & tst, nixgrpc::PushProcess & push)
{
    ...
    std::thread([&]() -> void { push.pushWait({"/nix/store/x.drv"}, 0); }).join();
    {
        // The mock acks after FARM_MOCK_HB (0.2 s). A full cancelPoll second meant a missed wakeup.
        constexpr std::chrono::milliseconds wellUnderCancelPoll{800};
        auto const started = std::chrono::steady_clock::now();
        push.pushWait({"/nix/store/quick"}, 0);
    ...
    push.pushWait({"/nix/store/y", "/nix/store/y2"}, someToken);
    {
        ...
        std::thread other([&]() -> void { push.pushWait({"/nix/store/shared"}, 0); });
        push.pushWait({"/nix/store/shared"}, 0);
        other.join();
    }
    {
        ...
        for (int i = 0; i < writers; i++) {
            threads.emplace_back([&]() -> void { push.pushWait(many, 0); });
        }
